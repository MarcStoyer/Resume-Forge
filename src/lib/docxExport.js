import { Document, Packer, Paragraph, TextRun, AlignmentType, HeadingLevel, BorderStyle, LevelFormat } from "docx";
...
function p(text, opts = {}) {
  return new Paragraph({
    ...
    children: [new TextRun({
      ...
      size: opts.size ?? 22, color: opts.color, font: FONT,
    })],
  });
}
...
function jobHeader(en) {
  const left = new TextRun({ text: en.org, bold: true, size: 22, font: FONT });
  const right = new TextRun({ text: "\t" + (en.dates || ""), italics: true, size: 21, font: FONT, color: "555555" });
  return new Paragraph({
...
function jobSub(en) {
  ...
  const left = new TextRun({ text: en.role || "", italics: true, size: 21, font: FONT, color: "333333" });
  const right = new TextRun({ text: "\t" + (en.loc || ""), size: 21, font: FONT, color: "555555" });
  return new Paragraph({
    spacing: { after: 40 },
    tabStops: [{ type: "right", position: 9360 }],
    children: [left, right],
...
export async function exportDocx(resume, template) {
  ...
  if (bits) children.push(p(bits, { align: AlignmentType.CENTER, size: 20, color: "555555", after: 120 }));

  // profile
  if (resume.profile.on && resume.profile.text.trim()) {
    children.push(p(resume.profile.text, { size: 21, after: 100 }));
  }
  ...
  for (const sec of resume.sections) {
    if (sec.kind === "entries") {
      ...
      for (const en of vis) {
        ...
        if (en.sub) children.push(p(en.sub, { italics: true, size: 20, color: "777777", after: 40 }));
