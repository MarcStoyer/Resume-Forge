...
export default function Builder({
  resume, setResume, honesty, setHonesty, jd, setJd, setCoverLetter, openCoverTab,
}) {
  ...
  // Select all / none across a section
  ...
  // Select all / none of an entry's bullets
  ...
  async function rewriteBullet(secId, enId, bullet) {
    ...
    try {
      ...
      const system = "You rewrite a single résumé bullet to better match a job description. " + honestyDir +
        " Return ONLY a JSON object: {\"text\":\"<rewritten bullet>\"}. No markdown, no commentary.";
      ...
      } else setErr("Couldn't parse the rewrite — try again.");
  ...
  async function genSummary() {
    ...
    try {
      ...
      const system = "You write a 2-3 sentence professional summary for a résumé. " + honestyDir + " " +
  ...
  async function onFile(e) {
    ...
    try {
      const system = 'Extract this resume into JSON with this exact schema: {"contact":{"name":"","location":"","email":"","phone":"","linkedin":"","github":""},"profile":"","experience":[{"org":"","role":"","dates":"","loc":"","bullets":[]}],"education":[{"org":"","role":"","dates":"","loc":"","bullets":[]}],"skills":[{"label":"","text":""}],"certs":[],"projects":[{"label":"","text":""}]}. Include EVERY job and EVERY bullet from the source. Do not truncate. Return ONLY the JSON, no markdown, no commentary.';
      ...
      if (name.endsWith(".pdf")) {
        ...
        content = [
          ...
          { type: "text", text: "Extract this resume to JSON per the schema. Include every job and every bullet." },
        ];
      } else if (name.endsWith(".docx")) {
        const buf = await readFile(file, "arrayBuffer");
        const out = await mammoth.extractRawText({ arrayBuffer: buf });
        content = "Extract this resume to JSON per the schema. Include every job and every bullet:\n\n" + out.value;
      ...
      const data = await callClaude({ system, messages: [{ role: "user", content }], max_tokens: 6000 });
      const parsed = extractJSON(extractText(data));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => fileRef.current && fileRef.current.click()} disabled={uploading} className="px-3 py-2 rounded-md text-sm font-medium border border-stone-300 hover:bg-stone-50 disabled:opacity-50">
          {uploading ? "Parsing your CV…" : "📥 Upload CV (PDF / DOCX / LinkedIn export)"}
        </button>
        <div className="text-xs text-stone-500 ml-auto">{selectedCount} bullets selected</div>
      </div>
      ...
      <div className="text-xs text-stone-400 px-1 pb-6">
        Tip: drag the ⋮⋮ handles to reorder sections, jobs, and bullets. Sections auto-save in this browser. <b>JD</b> button rewrites a single bullet for the pasted job description (using your current honesty level).
      </div>
    </div>
  );
}
