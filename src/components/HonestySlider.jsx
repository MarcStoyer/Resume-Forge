...
export default function HonestySlider({
  value, onChange, compact = false,
  title = "Honesty",
  hint = "Controls every AI rewrite, summary, and cover letter. 75 = reword without inventing. 100 = no changes.",
}) {
  ...
  return (
    <div className={compact ? "" : "bg-white rounded-lg border border-stone-200 p-4"}>
      {!compact && <div className="text-xs font-semibold uppercase tracking-wide text-stone-400 mb-2">{title}</div>}
      ...
      <div className="flex items-center justify-between mt-1">
        ...
        {value <= 35 && <div className="text-[10px] text-red-600 font-semibold">⚠ may fabricate</div>}
      </div>
      {!compact && (
